        ...
        <tr v-for="(driver, index) in driversBolt" :key="index">
          ...
          <div class="section">
            ...
            <div class="summary" v-show="fileMatrixUber != ''">
              <br>
                      <div class="input-title-content">Sprzedaż w okresie - {{ uberWeekBalance}}</div>
                      <br>
              <div>Przelew: {{uberWeekBank}}</div>
              <div>Gotówka: {{uberWeekCash}}</div>
              <br>
              <div @click="showUber = !showUber" class="btn">Pokaż szczegóły kierowców Uber</div>
              </div>
...
const addUber = (event) => {
  ...
  reader.onload = (e) => {
    ...
    uberWeekBalance.value = parseFloat(driversUber.reduce((sum, driver) => sum + driver.Utarg, 0).toFixed(2));
    uberWeekBank.value = parseFloat(driversUber.reduce((sum, driver) => sum + driver.Przelew, 0).toFixed(2));
    uberWeekCash.value = parseFloat(driversUber.reduce((sum, driver) => sum + driver.Gotówka, 0).toFixed(2));
  };
  reader.readAsText(selectedFile, "UTF-8");
...
const addFreeNow = (event) => {
  ...
  reader.onload = (e) => {
    ...
    freeNowWeekBalance.value = parseFloat(driversFreeNow.reduce((sum, driver) => sum + driver.Utarg, 0).toFixed(2));
    freeNowWeekBank.value = parseFloat(driversFreeNow.reduce((sum, driver) => sum + driver.Przelew, 0).toFixed(2));
    freeNowWeekCash.value = parseFloat(driversFreeNow.reduce((sum, driver) => sum + driver.Gotówka, 0).toFixed(2));
  };
...
function driversSummary(driversBolt, driversUber, driversFreeNow) {
  ...
  function addOrUpdateDriver(driverArray, driver) {
    const existingDriverIndex = driverArray.findIndex(d => d.Kierowca === driver.Kierowca);
    if (existingDriverIndex !== -1) {
...
function calculateDrivers(){
  ...
  console.log("All drives w calcultae drivers", allDrivers.value)
}
